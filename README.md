...
현재는 실제 게임 tick loop 이전 단계까지 구현되어 있으며, `RoomStatus::Playing` 전환 이후 `GameManager`와 tick 기반 게임 로직을 붙이는 방향으로 확장할 예정입니다.
...
- `RoomStatus::Playing` is implemented, but actual `GameManager` and tick loop are not connected yet.
